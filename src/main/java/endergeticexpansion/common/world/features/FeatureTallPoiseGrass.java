 ...
 */
public class FeatureTallPoiseGrass extends Feature<NoFeatureConfig> {
	private static final Supplier<BlockState> POISE_BUSH_TALL = () -> EEBlocks.POISE_GRASS_TALL.get().getDefaultState();
	
	public FeatureTallPoiseGrass(Function<Dynamic<?>, ? extends NoFeatureConfig> config) {
		super(config);
	}
	...
	public boolean place(IWorld worldIn, ChunkGenerator<? extends GenerationSettings> generator, Random rand, BlockPos pos, NoFeatureConfig config) {
		...
		for(int i = 0; i < 64; ++i) {
			...
			if (!isNearBolloomBud(worldIn, blockpos) && worldIn.isAirBlock(blockpos) && blockpos.getY() < worldIn.getWorld().getDimension().getHeight() - 2 && POISE_BUSH_TALL.get().isValidPosition(worldIn, blockpos)) {
				((BlockPoiseGrassPlantTall) POISE_BUSH_TALL.get().getBlock()).placeAt(worldIn, blockpos, 2);
				flag = true;
			}
		}
