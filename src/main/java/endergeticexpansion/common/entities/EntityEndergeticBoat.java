...
public class EntityEndergeticBoat extends BoatEntity {
	private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.createKey(EntityEndergeticBoat.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.createKey(EntityEndergeticBoat.class, DataSerializers.VARINT);
	private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.createKey(EntityEndergeticBoat.class, DataSerializers.FLOAT);
	private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.createKey(EntityEndergeticBoat.class, DataSerializers.VARINT);
	private static final DataParameter<Boolean> field_199704_e = EntityDataManager.createKey(EntityEndergeticBoat.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> field_199705_f = EntityDataManager.createKey(EntityEndergeticBoat.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Integer> ROCKING_TICKS = EntityDataManager.createKey(EntityEndergeticBoat.class, DataSerializers.VARINT);
	...
	public EntityEndergeticBoat(EntityType<? extends BoatEntity> type, World world) {
		super(type, world);
		this.preventEntitySpawning = true;
	}
		
	public EntityEndergeticBoat(World worldIn, double x, double y, double z) {
		this(EEEntities.POISE_BOAT, worldIn);
	...
	public EntityEndergeticBoat(FMLPlayMessages.SpawnEntity spawnEntity, World world) {
		this(EEEntities.POISE_BOAT, world);
	}
	...
	protected void registerData() {
		...
		this.dataManager.register(BOAT_TYPE, Type.POISE.ordinal());
        ...
        if (!this.isPassenger()) {
            if (onGroundIn) {
                if (this.fallDistance > 3.0F) {
                    if (this.status != EntityEndergeticBoat.Status.ON_LAND) {
