...
public class PurpoidTeleportToFlowerGoal extends AbstractPurpoidTeleportGoal {
	...
	protected void beginTeleportation(PurpoidEntity purpoid, BlockPos pos) {
		...
		var nearbyPurpoids = purpoid.level.getEntitiesOfClass(PurpoidEntity.class, purpoid.getBoundingBox().inflate(12.0D, 12.0D, 12.0D));
		for (PurpoidEntity nearbyPurpoid : nearbyPurpoids) {
			PurpoidEntity leaderOfNearbyPurpoid = nearbyPurpoid.getLeader();
	...
	public static boolean isRestingPosNotBlocked(PurpoidEntity purpoid, Vec3 pos, BlockPos restingPos, Direction side) {
		Level level = purpoid.level;
